...
const getVideosCacheKey = (channelId: string, page: number, size: number) =>
  `vods:${CACHE_POLICY.worker.chzzk.vods.version}:${channelId}:${page}:${size}`;

const getClipsCacheKey = (channelId: string, size: number) =>
  `clips:${CACHE_POLICY.worker.chzzk.clips.version}:${channelId}:${size}`;
...
export const fetchChzzkVideosBatch = async (
  ...
  return fetchCachedBatch<ChzzkVideosContent>({
    ...
    freshTtlMs: CACHE_POLICY.worker.chzzk.vods.freshTtlMs,
    staleTtlMs: CACHE_POLICY.worker.chzzk.vods.staleTtlMs,
...
export const fetchChzzkClipsBatch = async (
  ...
  return fetchCachedBatch<ChzzkClipsContent>({
    ...
    freshTtlMs: CACHE_POLICY.worker.chzzk.clips.freshTtlMs,
    staleTtlMs: CACHE_POLICY.worker.chzzk.clips.staleTtlMs,
    forceRefresh: options.forceRefresh === true,
