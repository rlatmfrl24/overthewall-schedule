...
import { createHandleMembers, D1MemberReader } from "../index";

const handleMembers = createHandleMembers(
  (env) => new D1MemberReader(getDbMock(), env.ASSET_BUCKET),
);
...
describe("members route", () => {
  ...
  it("활성 멤버 목록과 public cache 계약을 반환한다", async () => {
    const member = makeMember();
    getDbMock.mockReturnValue(makeDb([rowsQuery([member])]));
