...
import { D1MemberPostsPort } from "../infrastructure/d1-member-posts-port";
import { createMemberPostsHandler } from "./handler";
...
const handleMemberPosts = createMemberPostsHandler({
  getMemberPosts: (env) =>
    new GetMemberPosts(
      new D1MemberPostsPort(
        env,
        {
          extractHandle: (url) =>
            url?.split("/").filter(Boolean).pop() ?? null,
          fetchPosts: fetchXPostsForHandlesMock,
          isApiError: (error) =>
            error instanceof Error && error.name === "XApiError",
        },
        readStoredNaverCafePostsForSourcesMock,
      ),
    ),
});
...
describe("member-posts aggregate worker route", () => {
  ...
  it("관리자 cache-buster GET도 X 수집을 강제하지 않는다", async () => {
    fetchXPostsForHandlesMock.mockResolvedValueOnce({
      posts: [],
      byHandle: [{ handle: "otw_member", posts: [], error: null }],
    });
