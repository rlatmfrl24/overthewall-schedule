...
vi.mock("../infrastructure/naver-cafe-collector", () => {
...
import { createD1NaverCafeApplication } from "../infrastructure/d1-naver-cafe-application";
import { createNaverCafeHandler } from "./handler";
...
describe("naver cafe posts route cache", () => {
  ...
  it("관리자 cache-buster GET도 저장된 게시글만 조회한다", async () => {
    const env = makeEnv();
    ...
    expect(readStoredNaverCafePostsForSourcesMock).toHaveBeenCalledWith([source], {
      cacheDb: env.otw_db,
      size: 10,
    });
    expect(collectNaverCafePostsForSourcesMock).not.toHaveBeenCalled();
