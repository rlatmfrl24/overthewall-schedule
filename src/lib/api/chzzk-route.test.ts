...
describe("CHZZK media routes", () => {
  ...
  it("채널을 정규화·중복 제거하고 등록된 승인 프로필만 D1 저장 대상으로 표시한다", async () => {
    const { env } = makeEnv();
    const response = await handleVods(
      new Request(
        `https://example.com/api/vods/chzzk?channelIds=${registeredChannelId.toUpperCase()},${unknownChannelId},${registeredChannelId}&page=0&size=10`,
      ),
      env,
    );
  ...
  it("클립 size=10은 등록 채널에만 D1 캐시를 허용한다", async () => {
    const { env } = makeEnv();
    const response = await handleVods(
      new Request(
        `https://example.com/api/clips/chzzk?channelIds=${registeredChannelId},${unknownChannelId}&size=10`,
      ),
      env,
    );
    ...
    expect(fetchChzzkClipsBatchMock).toHaveBeenCalledWith(
      [
        ...
        { channelId: unknownChannelId, size: 10, cacheable: false },
      ],
      env.otw_db,
    );
  });
