...
const createEmptyPolicy = (
  ...
  apiPath: withRouteSearch(
    apiRoutes.memberPosts.read.build(),
    `sources=${source}&admin=1`,
  ),
});
...
export function useMemberPosts(
  ...
  return {
    ...
    x: {
      ...
      error: data.x.error,
      stale:
        data.x.byHandle.some((item) => item.stale) ||
    ...
    naverCafe: {
      ...
      stale:
        data.naverCafe.sources.some((source) => source.stale) ||
        (hasData && requestFailed),
