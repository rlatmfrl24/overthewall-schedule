...
export function DDayManager() {
  ...
  const ddaysQuery = useQuery<DDay[]>({
    ...
    queryFn: () => fetchDDays({ noCache: true }),
    staleTime: QUERY_STALE_TIME_MS,
  });
  const ddays = useMemo(() => ddaysQuery.data ?? [], [ddaysQuery.data]);
  const isFetching = ddaysQuery.isFetching;

  useEffect(() => {
    if (!ddaysQuery.error) return;
    console.error("Failed to load d-days:", ddaysQuery.error);
    toast({
      variant: "error",
      description: "D-Day 목록을 불러오지 못했습니다.",
    });
  }, [ddaysQuery.error, toast]);
