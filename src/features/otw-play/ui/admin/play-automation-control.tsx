...
export function PlayAutomationControl({ monitors }: { monitors: OtwPlayChannelMonitorDto[] }) {
  ...
  const teardownCount = monitors.filter((monitor) => monitor.subscription &&
    monitor.subscription.status !== "unsubscribed").length;
  ...
  const changePause = async () => {
    ...
    try {
      ...
      toast({ variant: "success", description: paused
        ...
        : "Play 자동화를 일시 중지했습니다. 구독 해제 확인·재시도와 데이터 보존 정리는 계속합니다." });
    } catch {
  ...
  return <section className="space-y-2 rounded-xl border p-3" aria-labelledby="play-automation-title">
    ...
    <p className="text-sm text-muted-foreground">전체 일시 중지는 채널 감시와 WebSub 구독, 자동 수집·소스 점검을 멈춥니다. 기존 곡·후보 검수와 데이터 보존 정리는 유지합니다.</p>
    {paused && <p role="status" className="text-sm">구독 해제 확인·정리 대상 {teardownCount}개. 재개 시 채널 감시를 개별적으로 켜야 합니다.</p>}
