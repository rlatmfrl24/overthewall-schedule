...
export function OtwPlayDefaultPlaylistPage({ playlistKey }: { playlistKey: string }) {
  ...
  if (defaults.isPending) return <div className="playlist-page playlist-detail"><PlaylistBackButton /><PlaylistDetailSkeleton /></div>;
  if (!playlist) return <div className="playlist-page"><p>{defaults.isError ? "목록을 불러오지 못했습니다." : "플레이리스트를 찾을 수 없습니다."}</p>
    {defaults.isError && <Button onClick={() => void defaults.refetch()}>다시 시도</Button>}<PlaylistBackButton /></div>;
  return <div className="playlist-page playlist-detail"><PlaylistBackButton /><header className="playlist-heading playlist-detail-heading"><img className="h-16 w-24 shrink-0 rounded object-cover" src={playlist.imageUrl || "/images/otw-play/glass-note.png"} alt="" onError={event => { if (!event.currentTarget.src.endsWith("/images/otw-play/glass-note.png")) event.currentTarget.src = "/images/otw-play/glass-note.png"; }} /><div className="playlist-detail-copy"><p>기본 플레이리스트</p><h1>{playlist.title}</h1><p>{playlist.description}</p>
...
function PersonalPlaylist({ playlistId }: { playlistId: string }) {
  ...
  if (saved.isPending) return <div className="playlist-page playlist-detail"><PlaylistBackButton /><PlaylistDetailSkeleton /></div>;
  if (!playlist) return <div className="playlist-page"><p>목록을 찾을 수 없거나 접근할 수 없습니다.</p>{saved.isError && <Button onClick={() => void saved.refetch()}>다시 시도</Button>}<PlaylistBackButton /></div>;
  const byId = new Map(resolved.data?.items.map(item => [item.performance.id, item]));
  return <div className="playlist-page playlist-detail"><PlaylistBackButton /><header className="playlist-heading playlist-detail-heading"><img className="h-16 w-24 shrink-0 rounded object-cover" src={playlist.imageUrl || "/images/otw-play/glass-note.png"} alt="" onError={event => { if (!event.currentTarget.src.endsWith("/images/otw-play/glass-note.png")) event.currentTarget.src = "/images/otw-play/glass-note.png"; }} /><div className="playlist-detail-copy"><p>나만 볼 수 있는 플레이리스트</p><h1>{playlist.title}</h1><p>{playlist.description}</p><p>가창 {playlist.itemCount}개</p></div>
