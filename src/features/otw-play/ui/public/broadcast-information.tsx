...
export function BroadcastInformation({ broadcast }: { broadcast?: OtwPlayBroadcastMetadata | null }) {
  return <dl className="grid gap-1 text-sm text-muted-foreground">
    ...
    <div><dt className="inline">방송일 </dt><dd className="inline">{broadcast?.performedOn ?? "미확인"}</dd></div>
