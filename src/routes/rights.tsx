...
const summaryItems: NoticeItem[] = [
  {
    title: "비공식 팬 운영",
    description:
      "이 사이트는 일정 확인을 돕기 위한 팬 운영 서비스이며, 오버더월 또는 각 소속사/플랫폼을 대표하지 않습니다.",
    icon: Info,
  },
  {
    title: "콘텐츠 권리 귀속",
  ...
  },
  {
    title: "개인정보 최소 처리",
    description:
      "서비스 운영에 필요한 로그인 상태와 설정 정보만 제한적으로 처리합니다.",
    icon: Shield,
  },
];

const privacyItems: NoticeItem[] = [
  {
    title: "로그인",
    description:
      "회원 기능 제공을 위해 Clerk 로그인 상태를 사용합니다. 비회원에게 공개된 기능은 로그인 없이 사용할 수 있습니다.",
    icon: Shield,
  },
  {
    title: "운영 설정",
    description:
      "일정, 공지, 멤버 정보 같은 서비스 데이터는 Cloudflare Worker와 D1을 통해 제공됩니다.",
    icon: Cloud,
  },
  {
    title: "외부 서비스 이동",
    description:
      "CHZZK, YouTube, X, NAVER Cafe 등 외부 링크를 열면 해당 서비스의 정책이 적용됩니다.",
    icon: ExternalLink,
  },
];
...
const contentNotices = [
  ...
  "외부 플랫폼에 표시되는 영상, 채팅, 게시글, 카페 글은 각 플랫폼의 제공 방식과 정책을 따릅니다.",
];
...
const platformNotices: RightsNotice[] = [
  ...
  {
    name: "X",
    notice: "X is a trademark or registered trademark of X Corp. X © X Corp.",
    href: "https://x.com/",
    icon: AtSign,
  },
];
        ...
        <header className="flex items-start gap-3 md:gap-4">
          ...
          <div className="min-w-0 space-y-3">
            ...
            <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground md:text-base">
              오버더월 스케줄표의 콘텐츠 권리, 외부 플랫폼 고지, 서비스 운영에
              필요한 개인정보 처리 범위를 안내합니다.
            </p>
            <p className="text-xs text-muted-foreground">
              시행일 및 최종 업데이트: 2026년 7월 6일
            </p>
          </div>
        </header>

        <NoticeGrid title="핵심 고지" icon={FileText} items={summaryItems} />
        <NoticeGrid title="개인정보 처리 안내" icon={Shield} items={privacyItems} />
...
function NoticeGrid({
  ...
  return (
    <section aria-labelledby={`${title}-title`} className="space-y-3">
      <SectionTitle id={`${title}-title`} icon={icon} title={title} />
...
function SummaryNotice({ item }: { item: NoticeItem }) {
