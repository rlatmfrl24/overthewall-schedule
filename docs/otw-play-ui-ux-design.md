...
상태: 개인 프로필·Play 멤버 SEO 구현, 큐레이션 다음 개발, 방송 가창·개인 감상 탭 후속 설계

기준일: 2026-09-08

현재 신규 목표는 DEC-080의 기본 큐레이션이다. 기존 발견 화면의 멤버·최근 곡 사이에
...
### 7.6 기본 큐레이션 — 신규 설계·미구현

확정 배치는 `추천 배너 → 멤버로 찾기 → 큐레이션 → 최근 공개된 곡`이며 기본 클릭은
해당 목록의 대기열 끝 추가다. 아래 카드 수·표현과 편집 방식은 권장안이다.

- 카드 3~4개로 시작하고 제목·짧은 선정 기준·이미지와 `대기열에 추가`를 표시한다.
  조건형은 `최근 공개순 최대 24곡`처럼 기준/상한을 안내한다. 조회하지 않은 정확한 곡 수나
  소요 시간을 확정 수치로 표시하지 않는다. 초기 개수와 상한은 TBD-020/021에서 결정한다.
- 모바일은 한 줄 가로 스크롤을 권장한다. 키보드로 카드에 접근하고 초점이 화면 안에
  보이도록 하며 색상·hover만으로 동작을 전달하지 않는다. 기존 Play 토큰과 테마를 사용한다.
- 카드 클릭은 곧바로 추가 요청이며 별도 상세 화면 이동을 필수로 하지 않는다.
  후속 편집/미리보기 버튼은 추가 버튼과 별도 컨트롤로 제공한다. 중첩 button을 만들지 않는다.
- 처리 중인 카드에 로딩/중복 클릭 방지를 제공하고 결과는 한 번 안내한다. 성공 시 현재 곡을
  방해하지 않는 `대기열 보기` 진입점을 제공할 수 있다. 대기열을 강제로 펼쳐 화면을 이동시키지 않는다.
- 정상 추가·전체 중복·정상 0곡·일부 재생 불가·조회 상한·통신 실패를 구별한다.
  실패는 기존 화면과 큐를 보존하고 해당 작업의 재시도를 제공한다. 다른 섹션을 오류로 대체하지 않는다.
- 현재 재생과 기존 큐 순서를 유지하고 빈 큐도 자동 재생하지 않는다. 결과는 실제 반영 수를
  기준으로 `18곡 추가 · 중복 4곡 · 재생 불가 2곡 제외`처럼 안내하고 `aria-live`로 전달한다.
- 추가된 곡은 기존 플레이큐에서 삭제·위/아래 이동한다. 후속 `내 큐레이션으로 만들기`는
  기본 목록의 독립 사본을 편집하는 안이다. 일시 큐 편집을 영구 저장처럼 표시하지 않는다.
- 사용자 저장 화면은 저장 범위 결정 후 목록 재열기·수정·삭제·미저장 보호·저장 결과 및
  충돌 시 입력 보존까지 설계한다. 빈 보관함 탭이나 동작하지 않는 편집 버튼을 먼저 노출하지 않는다.

실제 발견 진입→추가→대기열 확인·편집→세션 복원을 데스크톱·모바일·키보드에서 검증한다.
사용자 영구 저장은 별도 서버 재조회·재방문 검증이 필요하며 본 문서화 시점에는 미구현이다.
...
2. **곡과 참여자**: 곡명, 새 곡 장르(분류), 원곡 가수, 기존 곡 후보, 참여자 복수 선택, 외부
   참여자 텍스트, 선택 메모
3. **검토 후 제출**: 입력한 곡·장르(분류)·원곡 가수·참여자와 video ID·썸네일 확인,
...
원곡 가수 snapshot을 기본값으로 채운다. 새 곡은 추천값과 직접 입력으로 장르(분류)를
최대 10개까지 추가·삭제하고, 기존 곡 연결에서는 카탈로그에 저장된 분류를 유지한다. 현재 멤버는 이름·code·unit을 keyboard로
...
최종 화면은 thumbnail, 새 곡/기존 곡 구분, 새 곡 장르(분류), 원곡 가수와 참여자 chip, 메모 글자 수와
