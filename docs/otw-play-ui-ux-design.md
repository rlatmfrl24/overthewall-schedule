...
카탈로그의 draft 가창에는 `삭제`를 제공한다. 곡 단위 삭제는 연결된 가창이 없거나
모두 draft일 때만 활성화하며, 게시·철회 이력이 있으면 철회·보관 안내와 함께
비활성화한다. 삭제 전 confirm dialog에는 되돌릴 수 없다는 점과 함께 삭제될 draft
가창 수를 표시한다. 성공 뒤에는 optimistic removal 대신 catalog를 다시 읽는다.
...
- draft 가창과 draft-only 곡은 명시적 확인 뒤 삭제할 수 있고, 게시·철회 이력은 삭제 control로 제거할 수 없다.
