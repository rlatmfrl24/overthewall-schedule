...
차기 UI 목표: 현재 기능을 재사용하는 `발견 / 곡 탐색 / 멤버` 3개 화면과 짧은 모션.
[화면 시안·자동 구성·모션 명세](./otw-play-three-screen-design.md)가 DEC-077의 상세 기준이다.
현재 구현 기준을 설명하는 아래 두 메뉴·멤버 rail 배치는 아직 runtime에 존재한다.
차기 목표는 에디터·신규 노래책·보관함·추천 API를 요구하지 않으며 앱 구현 완료를 뜻하지 않는다.
