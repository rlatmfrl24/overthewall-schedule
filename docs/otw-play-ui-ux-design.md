...
- 가창자는 메인 보컬을 먼저 표시하고 서브 보컬·코러스·기타 참여는 `+N` 보조 정보로 축약한다. 보조 정보는 접근 가능한 이름과 역할 설명을 유지한다.
...
- Now Playing과 queue는 메인 보컬 이름·avatar를 우선 표시하고 나머지 credit은 낮은 강조도의 `+N`으로 제공한다.
