...
> 2026-09-07 공개·회원 UI 활성 기준: [OTW 3색 전면 개편](./otw-play-redesign-2026-09.md).
> 아래의 이전 시안·우측 레일·2개 탭·하단 콘솔 금지 지침은 개편으로 대체되었다. 서버·권한·데이터 계약과 관리자 운영 지침은 유지한다.
...
| 멤버 | `/play/members` — 클라이언트 경로 제안·미구현 | current member facet과 메인 보컬 필터 결과를 같은 카탈로그로 표시 |
...
멤버 선택은 해당 멤버 UID와 `participantRole=vocal`을 함께 적용한 곡 검색으로 이동한다.
...
- 멤버: 기존 facet의 멤버를 선택하면 `participantRole=vocal` 조건의 목록으로 바로 연결한다.
...
- 모든 현재 멤버가 최근 공개곡보다 먼저 한 줄 rail에 표시되고, 멤버 진입은 메인 보컬 검색으로 연결된다.
