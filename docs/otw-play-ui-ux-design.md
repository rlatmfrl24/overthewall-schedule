...
- 가창자는 메인 보컬 이름만 먼저 표시하고, 서브 보컬·코러스·기타 참여가 존재하면 역할별 보조 칩을 한 번씩 표시한다. 역할 칩의 hover·keyboard focus에서 해당 역할의 참여자 이름을 제공하며 인원수만 나타내는 `+N` 표기는 사용하지 않는다.
...
- Now Playing과 queue는 메인 보컬 이름·avatar를 우선 표시하고 나머지 credit은 낮은 강조도의 역할 칩으로 제공한다. 역할 칩은 hover·keyboard focus에서 해당 참여자 이름을 공개한다.
...
- 제출자는 자신의 상태만 볼 수 있다.
- 회원은 선택한 멤버와 외부 참여자마다 메인 보컬·서브 보컬·코러스·기타 참여 역할을 지정하고 최종 검토 화면에서 확인한다.
