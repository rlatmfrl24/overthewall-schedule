...
2. 영상 유형: `오리지널곡`, `공식 커버곡`, `노래방송` 중 하나를 선택한다. 오리지널·커버는 수동 곡 연결 없이 진행하고, 노래방송은 다곡·구간 연결 후속 범위 안내와 함께 현재 등록을 중단한다.
...
새 영상 진입점의 오리지널·커버는 YouTube metadata 제목으로 내부 song을 자동
생성한다. 별도의 기존 곡 검색·새 곡 form은 노출하지 않는다. 오리지널곡은 선택한
가창 참여자를 원곡 가수 credit으로도 사용하고, 공식 커버곡의 원곡 가수가 아직
확인되지 않았다면 빈 상태로 보존해 이후 `곡 정보 수정` 대상임을 검토 화면에
표시한다. `다른 가창 추가`에서 시작한 경우에만 선택한 기존 song을 그대로 재사용한다.
