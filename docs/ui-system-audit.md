...
아래 화면 행렬과 최초 검사 결과는 초기 구현 당시의 기록이다. 선행 백엔드·X 브랜치를 통합한 뒤의 재검증과 추가 수정은 문서 끝의 병합 검토 기록으로 구분한다.
...
- 백엔드 PR #124와 X PR #125가 포함된 master `07ca965`를 통합했다. 기존 미커밋 변경은 검토한 명시적 파일 목록으로 커밋했으며 Worker·API·DB 계약의 추가 변경은 없다.
- Play 검색의 필터 조회 실패가 아무 안내 없이 선택지 누락으로 보이던 문제와, 필터 로딩이 정상 곡 목록까지 막던 문제를 회귀 테스트 2건으로 재현했다. 필터 오류·로딩·재시도를 곡 목록과 독립시켰고 해당 파일 9개 테스트가 통과했다.
- 실제 일정 삭제 확인창을 Escape로 취소하면 포커스가 BODY로 빠지는 문제를 확인했다. 별도의 Radix Trigger가 없는 제어형 확인창도 열리기 직전 요소를 기억하도록 수정했다. 수정 전 회귀 FAIL, 수정 후 공통 확인·라우터 보호 테스트 3건 PASS, 같은 실제 화면에서 삭제 버튼 포커스 복귀 PASS.
- 런타임 수정 head `6419b24`를 Node 24.20.0 / pnpm 11.7.0, `http://localhost:5181`에서 확인했다. 390px·1440px × 라이트·다크 화면과 키보드 보기 전환, 멤버 필터 선택, 확인창 취소·승인, 입력 원복을 실행했다.
- 실제 공개 일정 편집 → 저장으로 로컬 D1 일정 `2645`(member 6, 2026-09-08, 제목 `브랜치 통합 검증 20260908-UI`)를 재조회했다. 이후 같은 화면에서 해당 검증 일정만 삭제하고 잔여 0건을 확인했다. 로컬 감사 이력은 유지한다.
- 실제 회원 제안 입력 → 주간 메뉴 이동에서 계속 편집 시 입력·포커스 보존, 변경 폐기 승인 후 주간 이동을 확인했다. Play 확인창 포털의 `otw-play-glass` 전달도 DOM에서 확인했다.
- 이 실행의 로컬 계정은 관리자 권한이 없어 정상 접근 거절을 확인했다. 관리자 표시의 최종 실제 검증은 운영 배포 후 기존 관리자 인증 경로로 이어간다. 운영 업로드·삭제·수집 실행·예산·주기 변경은 수행하지 않는다.
- 초기 문서의 PASS는 이번 최종 코드 검증을 대신하지 않는다. 최종 preflight 및 PR head·CI·병합·배포 결과는 PR 검증 댓글에 남긴다. 공급자 응답을 대체한 테스트는 외부 API 실제 성공이나 장기 비용 절감의 근거로 사용하지 않는다.
