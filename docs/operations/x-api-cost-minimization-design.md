...
사용자가 현재 비용을 수용 가능한 수준으로 판단하여 추가 비용 추적을 종료했다.
기존 9/2~10/2 관찰 계획과 7일·30일 비용 판정, 그 관측을 위한 자동화 지시는 종료한다.
이는 새 청구자료 대조나 정량 절감 목표 달성 판정이 아니다. 일반 게시글 수집, 사용량 기록,
예산 제한과 장애 확인은 유지하며 수집 간격을 자동 변경하지 않는다.
로컬 Codex 자동화 설정을 확인했으나 X 비용 또는 `x-30` 전용 예약은 없었다. 이번에 삭제한 예약은 없다.
아래 날짜가 있는 rollout·관찰·검증 결과는 과거 기록이다.

## 문서 상태

- 상태: 기존 rollout 기록은 당시 증거로 유지한다. 2026-09-08 답글 정책은 아래 9절을 따른다. 별도 브랜치 구현이며 운영 배포·실제 절감 확인은 후속 릴리스 범위다.
- 구현일: 2026-09-02
- Closeout readback: 2026-09-02 15:02 KST
- 이전 관찰 계획: 2026-09-02 ~ 2026-10-02. 추가 비용 추적은 2026-09-17 사용자 결정으로 종료.
...
| 7일·30일 비용 Closeout | 추적 종료 | 2026-09-17 현재 비용 수용에 따른 사용자 종료 결정. 추가 비용 판정 의무 없음 |

당시 관찰 계획은 빈 poll write, cache hit/miss, coalesced, guard와 비용 비교였다.
추가 비용 추적은 종료되었으며 `$9.50` 목표 달성이나 30분 주기 안정화를 새로 주장하지 않는다.
...
- 당시 관찰 automation `x-30`과 단계별 비용 판정 계획은 2026-09-17 사용자 결정으로 종료했다. 이 기록을 근거로 예약을 재생성하거나 수집 간격을 변경하지 않는다.

위 관찰 기간의 변경 제한·Console 대조는 당시 계획이다. 비용 추적 종료 후 추가 7일·30일 보고 의무로 사용하지 않는다. 이후 기능·운영 변경은 해당 작업의 승인 범위를 따른다.
...
## 9. 2026-09-08 답글 추가 조회 중단

현재 계약은 멤버의 답글 본문 수집을 유지하고, 답글 대상 Post/User의 추가 X 조회를
중단한다. 인용 보강, 기존 수집 주기, 예산 값과 설정 키는 유지한다. 앞 절의 유료
답글 복구·답글 재시도 설명은 과거 구현 기록이며 새 답글 정책에는 적용하지 않는다.

- 정상 수집과 공용 참조 보강에서 답글 전용 ID를 제외한다. 답글 URL을 인용으로
  추론하거나 rich link 경로로 우회 조회하지 않는다. 동일 ID가 인용에도 있으면
  인용에 필요한 조회는 유지한다.
- 저장 시 대상 ID를 중복 제거하고 기존 D1 게시물·유효 캐시를 연결한다. 작성자명은
  현재 타임라인의 `in_reply_to_user_id`와 기존 source/User 캐시로만 확인한다.
  캐시 미스는 `link_only`, `author_state=not_required`로 완료하며 예약·재시도가 없다.
- 기존 미리보기와 동일 대상 재수집의 JSON을 보존한다. 원문 숨김·제거 전파와 lease
  보호는 유지한다. 인용 보강의 대상 선택과 갱신은 인용에 한정해 답글 상태를 바꾸지 않는다.
- `0084_x-reply-stored-or-link.sql`은 generated custom data migration이다. 오래된 답글
  대기·오류를 정리하되 원문 JSON, 수집 이력, 실제 terminal 및 lease를 보존한다.
- 공개 피드는 확보한 미리보기를 유지하며, 없으면 확인된 멤버명 / 외부 계정 / 일반
  답글 관계와 `reply.postId` 직접 링크를 표시한다. 관계 표시는 대기·삭제 상태가 아니다.
  반복 context 조회와 재확인 버튼은 없다. 기존 context API의 인증·허용 대상 검사와
  D1 읽기 전용 계약은 유지한다.
- 관리자 설정은 ‘인용 원문 미리보기 모드’와 고정 답글 정책 안내로 구분한다.
  health의 `replyPolicy=stored_or_link`, `replyDisplay`는 미리보기 / 관계 표시 / 접근 불가
  건수다. 대기 합계·사유·오류·최장 대기·다음 시각은 인용만 포함한다.
- 새 보강 실행의 `scope=quotes`를 저장·읽기 DTO까지 전달하고 과거 혼합 실행과 구분한다.
  게시물 기록은 답글과 인용을 모두 보여준다. 기존 실행 JSON을 재작성하지 않는다.
- 비용 UI는 기존 원장의 사용량·예약·예산만 표시한다. 관계/캐시 건수를 금액으로
  환산하지 않는다. 운영 X 수집을 검증 목적으로 실행하지 않는다.

로컬 검증은 정상 수집 진입점의 답글 저장·cursor 진행, 공급자 호출·예약 부재,
인용의 예산·429·5xx·작성자 캐시 회귀, 재수집·대상 변경·lease·삭제 전파의 D1 통합,
생성 migration 전체 체인과 `pnpm preflight`, 실제 로컬 공개·관리자 화면을 포함한다.
후속 배포에서는 다음 정규 수집의 D1 readback과 기존 원장을 확인하고, 공급자 청구를
확인하기 전에는 실제 절감 달성으로 보고하지 않는다.

### 로컬 검증 증거 (2026-09-08)

- 기준: `origin/master`의 `f20a497`에서 생성한
  `codex/x-reply-reference-cost-reduction`, 별도 worktree
  `C:\Develop\overthewall-schedule-x-reply-cost`. 기존 UI 작업 변경은 포함하지 않았다.
- migration 84건을 임시 D1에 실제 적용해 검증하고 별도 worktree의 로컬 D1도 초기화했다.
  `0084`는 두 번 적용해 두 번째 변경 0건, 본문·미리보기·terminal·lease 보존을 확인했다.
- 단위 1,604건과 Worker D1 통합 231건 통과. 이 중 X 참조 회귀 39건은 정상 수집
  진입점의 본문 저장·cursor 진행, 추가 Post/User 호출·예산 예약 부재, 무료 Post/User
  캐시 재사용, 인용의 오류·재시도·동시성, 재수집·대상 변경·삭제 전파를 검증했다.
  공급자 응답은 테스트에서 대체했으며 운영 수집은 실행하지 않았다.
- `http://localhost:5178/feed`의 실제 앱과 로컬 D1 샘플로 멤버·외부·미확인 대상,
  기존 미리보기와 작성자 정보 없음, 대화 시작 글과 다른 직접 답글 링크를 확인했다.
- 실제 관리자 로그인으로 `/admin/collection?source=x`의 인용 전용 설정, 고정 답글
  안내, 답글 미리보기 2건·관계 표시 3건과 인용 오류 대기 1건의 분리, 신규 인용/과거
  혼합 실행 구분, 보관 기록의 답글·인용 동시 표시를 확인했다. 샘플 건수는 비용 절감액이 아니다.
- `pnpm preflight` 최종 통과: architecture, 타입, lint, 단위·D1, 통합 커버리지
  1,835건, build, local D1 doctor, mirror 16개 일치. 커버리지는 statement 80.12%,
  branch 67.27%, function 83.51%, line 81.63%다. 재확인 중 이번 변경과 무관한
  OTW Play 카테고리 테스트가 한 차례 5초 제한을 넘겼으나, 해당 코드나 시간 제한을
  바꾸지 않고 같은 전체 검사를 다시 실행해 통과했다.
- 운영 배포·정규 수집 후 D1 readback·공급자 청구에 따른 실제 절감 확인은 미수행이다.
