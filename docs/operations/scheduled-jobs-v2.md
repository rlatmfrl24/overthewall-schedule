...
5. 통합 `overthewall-schedule` Worker에 `X_BEARER_TOKEN`, `YOUTUBE_API_KEY`, Clerk, Analytics read token을 설정한다.
...
Naver → X → 일반 YouTube feed → ingestion → health/reconcile → auto-update →
retention 순서로 활성화한다. 구형 직접 실행 스케줄러는 제거되었고 현행 경로는
`Cron → Workflow → Outbox → Queue → 수집기`로 통일한다. 일반 YouTube 신규 업로드 feed는 이 플랫폼이 관리하고, 그 밖의
...
[2026-08-31 조정 기록과 재현 자료](../archive/youtube-cadence-adjustment-2026-08-31.md)를 참고한다. 현행 운영은 시간당 승인 채널 polling이며 WebSub는 종료되었다.

## 과거 기록 — 2026-09-01 정규 수집 안정화 Closeout
