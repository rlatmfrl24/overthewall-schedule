...
> 2026-09-09 운영 결정: DEC-078에 따라 WebSub을 종료하고 기존 업로드 목록 조회를 시간당 한 번 실행한다. 아래 WebSub 구독·갱신·해제·일일 recent-50 관련 기록은 이전 구현 이력이며 현재 운영 요구사항이 아니다. 승인·후보 검수·중지·generation 경계는 유지한다. [현재 운영 계약](operations/channel-upload-polling.md).

> 2026-09-07: 신규 전면 개편과 후속 UI 수정을 폐기하고 `2abb35f`의 기존 공개·회원 UI로 복원했다.
> 현재 기준은 발견·곡 검색과 우측 플레이어다. 기존 기능·권한·데이터를 보존하면서 디자인을 점진 개선한다.
> 아래 3개 화면 개편안은 이전 검토 기록이며 현재 구현 지침으로 사용하지 않는다.

상태: Living Baseline — 기존 UI 복원 완료, 기존 UI의 점진적 디자인 개선 방향
...
2026-09-05에는 사용자 요청으로 현재 기능을 재사용하는 `발견 / 곡 탐색 / 멤버`의
3개 화면과 모션 설계 목표를 반영했다. 에디터 큐레이션·새 노래책·저장은 이번 범위에서 제외한다.
현재 구현의 `발견 / 곡 검색` 두 메뉴를 이미 교체한 것은 아니다. [3개 화면 시안과 구현 범위](./otw-play-three-screen-design.md)를 함께 본다.
새 항목의 `[차기 UI]`는 설계 대상이며 기존 MVP 완료 판정을 소급 변경하지 않는다.
...
- 최근 공개 음악과 기존 관계·멤버 필터를 통해 원하는 목소리를 어떻게 쉽게 발견하는가?
...
| DEC-033 | 공개 Play의 중복 탐색 진입점을 `발견`과 `곡 검색` 두 개로 통합한다. | 차기 설계에서 부분 대체 | 현재 구현 기준선이다. DEC-077이 차기 메뉴 수와 발견·멤버 역할을 대체한다. `/play/discover`의 `/play` 호환 redirect, 관계 구분을 `/play/songs` URL 필터로 제공하는 계약은 유지한다. |
...
| DEC-035 | 발견은 반복 card surface보다 평면적인 music app 정보 밀도를 따른다. | 차기 설계에서 부분 대체 | 현재 구현은 최신 8곡 hero, 전체 멤버 rail, hero 포함 24곡 cursor 목록이다. 차기 배치는 DEC-077을 따르되 기존 최근 목록·대표 가창·첫 8곡 후보를 재사용한다. 실제 16:9 에셋을 유지하며 전체 cursor 탐색은 곡 탐색, 큰 멤버 선택은 멤버 화면으로 분담한다. |
...
| DEC-053 | playlist와 channel 자동화는 회원 proposal이 아닌 공유 system ingestion candidate를 생성한다. | 확정 | video identity와 discovery origin을 dedupe한다. playlist와 `singing_clip` candidate는 각각의 관리자 검수·승인 경계를 통과한 뒤 catalog draft로만 전환할 수 있다. 자동 publish, 가짜 submitter와 title 기반 권위 확정은 금지한다. |
...
| DEC-057 | channel 자동화는 OTW·멤버 공식 channel이 아니라 관리자가 승인한 노래 방송 clip channel의 신규 upload를 `singing_clip` system candidate로 수집한다. | 확정 | OTW·멤버 공식 영상은 관리자 단건·playlist로 직접 추가한다. clip candidate는 시간당 업로드 목록 조회, 자동 과거 소급 수집 0, title 기반 triage를 사용한다(DEC-078). 승인된 `approved_kirinuki` 채널 후보도 자동 publish하지 않으며 개별 관리자 검수 뒤 DEC-067의 비공개 draft로만 전환한다. |
...
| DEC-059 | playlist 후보 검수는 공통값 일괄 설정 대신 행별 보완과 즉시 적용 미리보기를 사용한다. | DEC-075로 화면 구조 보완 | 선택 checkbox는 사용하지 않는다. 곡 연결·신규 생성, 원곡 가수, 가창자·역할과 공개 분류는 행별 독립 popup에서 편집하고, 실제 저장 draft와 필수 누락 미리보기는 각 영상 아래의 가로 배치 영역에서 편집 중에도 즉시 갱신한다. 기존 곡은 곡명·별칭·원곡 가수로 검색하며 정확히 일치하는 곡이 없을 때 새 곡 입력으로 전환한다. `ready` 저장은 새 곡과 새 원곡 가수·외부 가창자 identity를 같은 트랜잭션에서 catalog에 생성해 다음 행에서 즉시 재사용하게 하고, performance draft 변환은 job 전체 ready 후보 일괄 처리로 유지한다. |
...
| DEC-066 | 승인 채널과 외부 주체는 하나의 관리 작업면에서 다루고, YouTube 채널 표시명은 입력한 채널 ID의 권위 조회 결과를 사용한다. | 확정 | 별도 `외부 주체 관리` Sheet와 채널 신규 등록의 소유·연결 주체 선택을 제거한다. 채널 ID 조회가 성공하면 표시명을 읽기 전용으로 자동 입력하며 조회한 ID와 현재 입력이 같을 때만 등록한다. 이미 등록된 채널의 편집 화면에서는 현재 연결 주체를 표시하고 잘못된 소유·소속 연결을 검색·교체·해제할 수 있으며, 명시적 확인 뒤 version CAS와 감사 event를 포함한 원자 갱신으로 저장한다. 이전 WebSub 상태·복구 계약은 DEC-078의 기능 종료로 대체하며 과거 기록은 보존한다. |
...
| DEC-071 | 관리자 catalog와 채널 감시 운영 화면은 다중 source, 명시적 실패, 권위 상태와 generation 경계를 보존한다. | 확정 | performance는 primary가 정확히 하나인 `sources[]`를 저장하며 Worker는 한 릴리스 동안 legacy `source`를 정규화한다. proposal·ingestion·monitor·observability 실패를 빈 목록과 구분하고 CAS 충돌은 입력을 보존한 `409 PLAY_ADMIN_STALE_WRITE`로 처리한다. 채널 자동 조회는 전역 허용·활성 monitor·유효 수집 승인에 한정하며 이전 generation 미처리 후보는 현재 inbox와 분리한다. |
...
| DEC-077 | 차기 주요 메뉴는 `발견 / 곡 탐색 / 멤버`로 나누되 현재 카탈로그·필터·상세·player를 재사용한다. | 확정: 설계 목표·미구현 | 발견은 최근 정렬과 첫 8곡 후보의 자동 구성, 곡 탐색은 전체 검색·버전 비교, 멤버는 current member facet과 메인 보컬 필터의 시각 탐색을 담당한다. P1 노래책·프로필·제작 참여·새 집계를 요구하지 않는다. 에디터·모음 편집·보관함·개인화 추천은 이번 범위에서 제외한다. 짧은 모션과 reduced-motion을 설계하며 단일 iframe·공개 권한·큐 의미를 보존한다. `otw-play-three-screen-design.md`가 세부 기준이다. |
| DEC-078 | 소수 승인 채널의 신규 업로드는 기존 uploads playlist 조회를 시간당 한 번 실행해 수집하고 WebSub을 종료한다. | 확정 | WebSub 구독·갱신·해제·delivery 처리와 일일 recent-50 예약을 제거한다. 승인·pause·CAS·lease·generation·250개 continuation·gap 감지·명시적 1~20개 backfill·검수 후보 저장은 유지한다. 재개는 최신 기준점 확보 뒤 원자적으로 수행하며 중지 기간을 자동 소급하지 않는다. 과거 구독·전달·작업 기록은 보존하고 공개 API와 공개 flag는 변경하지 않는다. |
...
차기 발견의 자동 구성은 현재 검수된 카탈로그의 최근 정렬과 대표 가창을 재사용한다.
개인 취향·인기도나 전체 신규 가창을 보장하는 추천으로 표현하지 않는다.
별도 에디터 게시·모음 편집 도구와 생성형 추천문은 이번 UI 개선에 추가하지 않는다.
...
### 9.8 현재 기능을 재사용하는 3개 화면 [차기 UI]

- FR-049: 발견은 기존 최근 목록·대표 가창·첫 8곡 후보를 자동으로 구성하고 실제 공개일과
  특정 가창 재생을 분명하게 보여야 한다. 수집일을 공개일로 대체하지 않는다.
- FR-050: 이어 볼 목록과 관계·멤버 진입은 현재 데이터와 필터를 사용한다. 로드된 표본으로
  전수 추천·인기·개인화를 주장하지 않으며 새 추천 API를 전제로 하지 않는다.
- FR-051: 큰 멤버 선택과 전체 cursor 탐색을 각각 멤버·곡 탐색으로 분담한다.
  에디터 모음·편집 관리·홍보문 게시·저장 기능 없이 의미 있는 탐색 흐름을 제공해야 한다.
- FR-052: 곡 탐색은 하나의 검색 입력과 기존 URL 조건을 사용하고 실제 상세에서 읽은
  가창을 비교하며 선택한 performance를 상세와 재생에서 보존해야 한다.
- FR-053: 멤버 화면은 current member facet과 `member`·`participantRole=vocal` 조건으로
  공개 곡을 보여준다. 노래책·프로필·집계·제작 참여와 FR-043~048은 별도 후속 범위다.
- FR-054: 세 화면과 곡 상세는 같은 현재 곡·player·세션 큐를 공유하고 DEC-036의
  재생·다음·마지막 추가 및 중복 방지 의미를 보존해야 한다.
- FR-055: 메뉴·필터·버전 펼침·조작 피드백의 짧은 모션과 reduced-motion 대안을 제공한다.
  모션이 API 응답·focus·화면 접근·재생을 지연하거나 iframe을 재생성하면 안 된다.

화면·재사용 계약·모션 수치는 [3개 화면 설계서](./otw-play-three-screen-design.md)를 따른다.
새 프런트엔드 배치와 상호작용은 필요하지만 API·DB·관리자 기능 확장은 전제하지 않는다.
...
- ADM-026: 각 playlist 후보의 행별 보완 popup에서 기존 곡을 검색하거나 검색 결과에 없는 새 곡과 원곡 가수·외부 가창자를 입력하고, 실제 저장될 곡·곡 라벨·영상 라벨·가창자·역할·공개 분류와 누락값을 해당 영상 아래의 가로 배치 영역에서 즉시 미리 볼 수 있어야 한다. `ready` 저장 시 새 곡과 새 외부 identity를 즉시 catalog에 생성해 다음 행에서 검색·재사용할 수 있어야 하며, job 전체의 ready 완료 후보는 performance draft로 일괄 변환할 수 있어야 한다.
...
- ADM-037: 멤버 참여 정보 정정 제안을 승인·거절할 수 있고 승인 변경은 event와
...
구분한다. playlist candidate는 관리자 검수 뒤 공식 영상 draft로 변환한다. `singing_clip`은
활성 `approved_kirinuki` 채널과 개별 검수 완료를 서버에서 다시 확인한 뒤 비공개
`broadcast` draft로만 변환하며 candidate 상태와 draft 생성을 원자적으로 반영한다.
...
- 방송별 세트리스트
- 신규 키리누키 영상 후보 자동 수집
- 승인된 노래 clip channel의 시간당 업로드 목록 조회 후보함
...
### 12.4 PR-9 closeout 이후 우선순위

2026-09-05의 3개 화면 개선은 현재 API와 기능의 사용성·미학·모션을 개선하는 별도 UI 범위다.
아래 P1 노래책·P2 큐레이션·저장은 그 전제가 아니며 우선순위도 변경하지 않는다.
...
| P0-B | 노래 clip channel 자동 후보함 | DEC-078의 시간당 uploads polling과 250개 cap·watermark·gap·generation·명시적 최근 1~20개 backfill을 사용한다. 신규 영상은 관리자 검수 뒤 비공개 broadcast draft로 전환한다. WebSub과 일일 recent-50 예약은 종료한다. 공개 전 전역 pause와 채널별 pause는 유지한다. |
...
### 14.1 차기 3개 화면 수용 기준 — 구현 전

1. 기존 최근 목록으로 자동 구성한 발견에서 가창자·공개일을 확인하고 정확한 버전을 재생한다.
2. 곡 탐색에서 URL 필터·가창 버전을 비교하고, 멤버 화면에서 현재 멤버의 메인 보컬 곡을 찾는다.
3. 세 화면과 곡 상세를 오가도 현재 곡·큐가 유지되고 iframe이 중복되거나 다시 시작되지 않는다.
4. 키보드로 검색·필터·버전 펼침·큐 조작을 완료하고 긴 제목·모바일에서 핵심 행동에 접근한다.
5. 모션이 빠른 연속 입력을 방해하지 않으며 reduced-motion에서 이동·확대 없이 같은 상태와 행동을 제공한다.
6. 신규 API·DB·관리자 편집 도구 없이 동작하며 정상 빈 목록과 조회 오류를 구별한다.

시안 이미지·fixture·하위 API만으로 위 사용자 흐름의 완료를 대신하지 않는다.
...
| 날짜 | 변경 |
| --- | --- |
| 2026-09-05 | 사용자 요청과 범위 축소 피드백에 따라 현재 카탈로그·필터·상세·player를 재사용하는 3개 화면 시안과 모션을 문서화했다. DEC-077, FR-049~055 및 차기 수용 기준을 추가했다. 에디터·새 노래책·저장은 이번 개선의 전제에서 제외하고 운영 코드·API·DB·flag·프로그램 우선순위는 변경하지 않았다. |
...
| 2026-09-03 | DEC-035 공개 탐색 밀도 보완. 발견 hero를 폭 기준 16:9로 전환하고 모든 현재 멤버 rail을 최근 공개곡보다 먼저 배치. 멤버 진입을 `participantRole=vocal` 검색으로 고정하고 최근 공개곡은 첫 8곡 hero 후보를 유지한 채 24곡 cursor 무한 스크롤로 확장. 곡 검색 필터는 URL 의미를 유지하는 기본 접힘 고밀도 disclosure와 접힘 밖 활성 필터 요약으로 통합 |
| 2026-09-03 | DEC-042·049 표시 계층 보완. 곡 검색 card와 우측 player에서 관계·공개 형태·참여 형태·게시일을 개별 상단 chip으로 통일하고 원곡 가수 접두 문구를 제거. player는 380px rail을 유지하면서 transport와 전체 폭 volume row를 분리하고 게시 채널 label도 chip으로 표시 |
| 2026-09-03 | DEC-042 볼륨 조작 보완. 별도 volume row 대신 volume trigger를 repeat·shuffle과 같은 기능 행에 통합하고, 클릭 시 위로 열리는 세로 slider popover 안에서 0–100 조절·현재 수치·음소거를 제공하도록 변경 |
