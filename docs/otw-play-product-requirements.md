...
현재 기준은 DEC-079/080, 9.7~9.10절과 [구현 가이드 29절](./otw-play-implementation-guide.md#29-다음-개발-우선순위와-문서-closeout)이다.
...
철회·삭제된 가창 참조는 개인 목록에 이용 불가 항목으로 남긴다. 비공개 카탈로그 정보는 반환하지 않는다. 개인 응답 no-store, 개인 페이지 noindex, 계정 전환 시 캐시·편집 상태 제거를 적용한다. 구현과 검증 경계는 [구현 가이드 28절](./otw-play-implementation-guide.md#28-큐레이션-요구사항과-구현-계획)을 따른다.
