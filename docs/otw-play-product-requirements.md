...
| DEC-035 | 발견은 반복 card surface보다 기준 이미지의 평면적인 music app 정보 밀도를 따른다. | 확정 | 첫 catalog page의 최신 8곡을 고정 hero 후보로 사용하고 hero media는 container 폭을 따르는 제한 없는 16:9로 표시한다. 그 아래 모든 현재 멤버를 한 줄 가로 rail에 먼저 배치하며 멤버 진입은 해당 멤버의 메인 보컬 곡 검색으로 연결한다. 최근 공개곡은 hero 곡을 포함한 전체 폭 구분선 table로 두고 24곡씩 cursor 기반 무한 스크롤하되, 중복 자동 호출을 막고 추가 로딩 실패·`IntersectionObserver` 미지원 환경에는 명시적 `더 불러오기`를 제공한다. |
