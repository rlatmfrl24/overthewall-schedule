...
| DEC-025 | 새 영상 등록은 metadata 확인 직후 `오리지널곡`, `공식 커버곡`, `노래방송`으로 유형을 먼저 확정한다. | 확정 | 오리지널·커버는 수동 곡 연결 화면을 건너뛰고 내부 song을 자동 생성한다. 오리지널의 원곡 가수는 선택한 가창 참여자를 재사용하고, 커버의 미확정 원곡 가수는 거짓 identity로 채우지 않는다. 다곡·구간 연결이 필요한 노래방송은 후속 범위로 표시하고 현재 command로 저장하지 않는다. |
