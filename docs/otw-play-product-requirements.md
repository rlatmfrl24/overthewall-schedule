...
큐레이션의 상세 구현 방향은 같은 가이드 28절에 있으며 기능은 아직 구현되지 않았다.
...
| DEC-080 | 발견의 멤버와 최근 노래 사이에 기본 큐레이션을 제공하고, 클릭하면 해당 곡 목록을 기존 재생 대기열 끝에 추가한다. | 확정: 요구사항·미구현 | 사용자에게 추가 위치를 확인했다. 기본 템플릿을 먼저 제공하고 사용자 편집을 후속 목표로 둔다. 조건형/고정 목록형, 최초 구성, 저장 범위는 TBD-020~023에서 구분하며 대기열 전체 교체를 기본 클릭으로 구현하지 않는다. |
...
### 9.9 기본 큐레이션과 사용자 편집 [신규 요구사항·미구현]

사용자 확정 요구사항:

- FR-056: 발견의 `멤버로 찾기`와 `최근 공개된 곡` 사이에 기본 큐레이션을 제공한다.
- FR-057: 큐레이션 클릭은 해당 곡 목록을 기존 재생 대기열 끝에 추가한다.
- FR-058: 기본 템플릿을 먼저 제공하고 향후 사용자가 편집할 수 있도록 설계한다.
  현재 대기열의 일시 편집과 큐레이션 자체의 편집·저장은 서로 다른 기능이다.

다음은 현재 코드에 근거한 **권장 구현 계약**이다. 사용자 확정 사항과 구분하며,
변경 필요 시 아래 미결정 항목과 구현 가이드를 함께 갱신한다.

- 현재 곡·진행 위치·기존 대기열 순서·반복 상태를 유지한다. 빈 대기열에는 목록만
  추가하고 자동 재생하지 않는다. 추가된 곡을 기존 섞기 상태 때문에 자동 재정렬하지 않는다.
- 기존 `performanceId` 중복 제거 의미를 유지한다. 다른 가창 버전은 별도 항목이며
  큐레이션 내부 및 기존 대기열과의 중복을 함께 제거한다.
- 템플릿 조건에 따른 실제 카탈로그 조회를 사용하고, 발견 화면에 로드된 일부 곡을
  전체 후보처럼 취급하지 않는다. 기존 공개 범위·관리자 preview·소스 선택 정책을 따른다.
- 목록 조회 성공 후 최신 대기열에 일괄 반영하며 실제 추가·중복·재생 불가 제외 수를
  알린다. 통신 실패를 재생 불가로 해석하지 않고, 조회 실패 시 대기열을 유지한다.
- 기본 템플릿, 실행된 대기열, 후속 사용자 큐레이션의 소유·수명·저장 경계를 분리한다.
  원본 템플릿 변경이 사용자의 편집본이나 이미 추가된 대기열을 덮어쓰지 않도록 한다.

상세 기본값, 구현 순서, 수용 기준과 후속 분석은 [구현 가이드 28절](./otw-play-implementation-guide.md#28-큐레이션-요구사항과-구현-계획)을 따른다.
조건형 템플릿과 기존 API 재사용이 초기 권장안이며, 운영자 고정 선곡·배포 없는 편집이
필수라면 저장 모델과 관리 API를 포함하도록 첫 전달 범위를 다시 산정한다.
...
- 저장형 사용자 큐레이션은 개인 감상 목록과 같은 소유·저장 모델로 연결하고 중복 보관함을
  만들지 않는다. 상세 schema/API·보관 기간·동기화·공개/공유는 아직 확정되지 않았다.
...
| 높음 · NEXT-CUR | 기본 큐레이션 | CUR-01~05: 발견 멤버 아래 기본 템플릿·대기열 끝 일괄 추가·결과 피드백·실제 검증. 사용자 편집·저장은 후속 단계 |
...
| TBD-020 | 최초 큐레이션 구성과 조건형/고정 목록형 | 조건형 3~4개 우선 권장. 오리지널·공식 커버·듀엣은 후보이며 실제 곡 분포·가창 선택을 확인해 확정 |
| TBD-021 | 큐레이션 곡 수·정렬·조회 예산 | 최근순 최대 24곡 권장. 페이지/후보 조회 상한과 부족·제한 도달 안내를 실제 데이터로 결정 |
| TBD-022 | 사용자 편집 대상과 보관 범위 | 기본 템플릿의 곡 목록을 내 사본으로 복사해 곡·순서 편집 권장. 조건 편집, 기기 내 저장/계정 저장, 공개/공유는 별도 결정 |
| TBD-023 | 운영자 선곡·수정과 태그/분위기 큐레이션 | 배포 없는 고정 목록 편집 필요 여부를 먼저 결정. 태그 필터의 곡/가창 범위·ANY/ALL·분류 품질 및 비용을 후속 분석 |
