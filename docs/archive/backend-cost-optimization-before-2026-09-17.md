> **과거 기록 (2026-09-17 보존)**: 정리 전 원문이다. 당시 완료·미완료·운영 수치와 폐기된 설계는 현재 상태를 뜻하지 않는다. 현재 계약은 [현행 문서](../operations/backend-cost-optimization.md)와 [개발 상태](../development-status.md)를 따른다. 명시적으로 대체하지 않은 요구사항과 무결성 제약은 삭제하거나 완료 처리하지 않는다.

# 무료 운영 우선 백엔드 최적화

> 과거 구현·배포 기록: 2026-09-08. 아래 WebSub 해제·복구 운영 계약은 2026-09-09 종료됐다. 현행 운영에서는 [시간당 채널 조회](../operations/channel-upload-polling.md)를 사용하며 Hub 확인을 기다리거나 구독 기능을 재개하지 않는다. 수치와 배포 증거는 당시 기록으로 보존한다.
...
### 2026-09-08 첫 관측 — UPDATE 비용 목표 미충족, WebSub 종료 미완료

> 후속 조치: Outbox 바깥 UPDATE PK 수정 `51a1182`가 2026-09-08 22:03:10 UTC에 Worker `9ac3aaf1-2ffd-4643-983f-acb7e8f2dafa`로 배포됐다. 운영의 같은 완료 run 대상 실제 UPDATE는 2,092행 → 5행, 두 경우 모두 쓰기 0이었다. 아래 첫 관측은 수정 전 기록이다. 후속 근거는 이 문서의 Outbox UPDATE PK 보완 절을 참조한다. WebSub·운영 COUNT/MIN은 이 수정에 포함되지 않는다.

실제 관측은 10:00 UTC(한국시간 19시)부터 수행했다. 현재 Worker는 `d4edde27-b2f3-42d3-a94e-834b0d079cfd`, 03:10:13 UTC에 100% 배포됐다. 초기 관측 버전과 다르다. 원본 저장소 HEAD `f692d83`은 비용 최적화 `8b2e300`을 포함하며 이후 X 답글 처리·UI 통합·YouTube 초기화 재시도 수정이 병합되어 있다. 운영 쿼리에서도 추가한 상태 인덱스 사용을 확인했다. Cloudflare version metadata에는 Git SHA가 없어 이 메타데이터만으로 배포 소스 커밋을 단정하지 않는다.

배포일 00시는 구버전·신버전과 초기 검증이 섞여 비교에서 제외했다. **전날과 같은 UTC 01:00~09:59의 9시간**을 비교한다. 트래픽과 후속 변경도 달라 순수한 단일 변경 효과로 해석하지 않는다.

| 지표 | 9/7 같은 9시간 | 9/8 같은 9시간 | 판정 |
| --- | ---: | ---: | --- |
| 기존 outbox claim·전달 복구 3종 쿼리 읽기 | 815,885 | 106,255 | 호출 225회 → 49회. 새 eligibility SELECT 70행을 포함해 약 87% 감소 |
| 실제 claim UPDATE 조회당 읽기 | 3,448.5 | 2,168.5 | 개선됐지만 10행 이하 목표와 큰 차이 |
| 새 eligibility SELECT | 해당 없음 | 10회 / 70행 | 평균 7행, 쓰기 0 |
| 운영 상태 COUNT·MIN 복합 SELECT | 같은 쿼리 형태 없음 | 39회 / 167,565행 | 평균 4,296.5행. 별도 최적화 대상 |
| D1 데이터베이스 Metrics 읽기 | 784,190 | 336,084 | 별도 Metrics 집계 |
| D1 데이터베이스 Metrics 쓰기 | 7,592 | 1,987 | 별도 Metrics 집계 |

쿼리별 adaptive 집계와 데이터베이스 Metrics는 실제로 합계가 일치하지 않아 합산하거나 정확한 점유율로 환산하지 않는다. 9/8 UTC 00:00~09:59 Metrics는 읽기 500,261행·쓰기 3,264행이다. 9/7 하루 Metrics 재조회는 읽기 2,071,221행·쓰기 16,460행으로 초기 감사 스냅샷과 차이가 있으며, 기존 세 outbox 쿼리 합계는 1,863,889행으로 동일하다. 첫날 일부 시간 관측으로 7일 목표 달성을 확정하지 않는다.

**검증 범위 정정:** 앞의 6행·1행 및 10행 이하 주장은 후보 탐색 SELECT의 결과다. 실제 운영 `UPDATE ... WHERE id IN (SELECT ...) RETURNING ...` 전체의 비용 달성을 증명하지 못했다. 읽기 전용 `EXPLAIN QUERY PLAN`에서 내부 SELECT는 상태 인덱스를 사용하지만 바깥 UPDATE는 `SCAN scheduled_outbox`였다. 운영의 오래된 `sqlite_stat1` 통계(PK 기준 22행)는 현재 보존 이력과 차이가 있어 계획 차이의 원인 후보다. 바깥 UPDATE에 기존 PK 인덱스를 명시한 대안은 EXPLAIN에서 PK SEARCH로 바뀌었지만 실제 변경·UPDATE 실행·ANALYZE는 하지 않았다. 후속 구현에서는 운영 통계 차이를 반영하고 실제 lease·CAS·RETURNING 경로 전체를 검증해야 한다.

- Play 전역 중지 `true`, monitor `paused` version 106·generation 0, 승인과 검수 대기 후보 2개 유지.
- WebSub 해제 유지보수는 01:14~09:14 UTC 9회 실행했고 모두 `hub_timeout`으로 실패했다. 최신 요청은 09:14:06, 오류는 09:14:26. 상태 `unsubscribing`, `pending_mode=unsubscribe`, lease 없음으로 유지한다. 실제 대상 1개와 세부 `ok:false`가 있으므로 이 9개 항목은 불필요한 유지보수가 아니며 허위 성공도 아니다. 허브 종료 확인은 여전히 미완료다.
- 중지 이후 source health·channel reconcile·recent reconcile·ingestion recovery·retention prune 새 run은 0이었다. 관측 범위에서 대상 없는 유지보수 항목은 발견하지 않았다. 공용 복구·정리를 실제 실행한 증거로 이 0을 해석하지 않는다.
- X 8개 소스는 07:54 UTC 성공, Naver 8개는 05:14~09:14 성공, YouTube 공식 8개는 09:24·키리누키 6개는 04:24 성공. 오류는 모두 0이고 최근 YouTube 중첩 결과도 실패·quota 보류 없이 성공했다. 미발송 outbox·queued/running item·만료 lease는 모두 0이다.
- 최신성 관측: X의 2시간 설정은 완료 시각과 cron 경계로 실제 2시간 30분 간격이었다. YouTube도 next-check와 시간 단위 cron 경계에 따른 추가 대기가 있다. Naver 하네는 저장된 next-check(3시간)와 활동량 기반 planner(6시간)가 다르다. 현재 실패나 예산 차단은 아니지만 주기·운영 표시 정합성은 후속 검토 사항이다.
- X 전체 내부 원장은 $0.365/$1.00이며 그 안의 미리보기 부분 원장은 $0.300/$0.300이다. 두 금액을 합산하거나 공급자 청구액으로 표시하지 않는다.

관측은 Cloudflare 조회, D1 SELECT·EXPLAIN과 로컬 문서 기록만 수행했다. 운영 데이터 쓰기·강제 수집·설정 변경·배포·전체 테스트는 수행하지 않았다. 성공한 상태 SELECT 17건은 2,314행, 추가 진단 SELECT 2건·EXPLAIN 5건은 2,306행을 읽었다. 합계 24건·4,620행이며 전부 `rows_written=0`, `changed_db=false`였다. 후속 비용·허브 연동 수정은 이 읽기 전용 관측의 범위에 포함하지 않았다.
