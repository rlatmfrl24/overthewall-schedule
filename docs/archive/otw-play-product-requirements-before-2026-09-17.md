> **과거 기록 (2026-09-17 보존)**: 정리 전 원문이다. 당시 완료·미완료·운영 수치와 폐기된 설계는 현재 상태를 뜻하지 않는다. 현재 계약은 [현행 문서](../otw-play-product-requirements.md)와 [개발 상태](../development-status.md)를 따른다. 명시적으로 대체하지 않은 요구사항과 무결성 제약은 삭제하거나 완료 처리하지 않는다.
...
> 2026-09-14 노래 클립 문서화: 공통 곡·가창·소스 모델을 재사용하면서 공식 곡과 노래 클립의 관리·탐색을 분리하는 구현 기준안을 DEC-081·FR-060~064에 정리했다. [노래 클립 요구사항과 구현 계획](../otw-play-singing-clips-requirements-and-plan.md)을 함께 따른다. 기능은 미구현이며 수록·공개 미결정 정책은 문서화만으로 확정하지 않는다. 기존 방송 draft 공개 제한은 실제 게시·조회·재생 확장이 완료되기 전까지 유지한다.
...
상태: Living Baseline — 개인 프로필·Play 멤버 SEO 구현, 기본 큐레이션 다음 개발
...
2026-09-08 사용자 우선순위 결정에 따라 **멤버 페이지 SEO와 기본 큐레이션**을 다음 개발
대상으로 올렸다. 멤버 제작 참여·대표곡 지정·참여 정보 정정은 낮은 우선순위로 분리한다.
방송 가창·개인 감상 기능은 **사용자용 OTW Play 내부 탭**에서 관리할 후속 기능이다.
현재 기준은 DEC-079/080, 9.7~9.10절과 [구현 가이드 29절](otw-play-implementation-guide-before-2026-09-17.md#29-다음-개발-우선순위와-문서-closeout)이다.
큐레이션은 2026-09-10 기본 모음과 회원 비공개 편집·저장을 구현했다. 현재 계약과 검증 범위는 같은 가이드 28절을 따른다.
2026-09-05의 DEC-077 및 `[차기 UI]` 3개 화면 개편안은 9월 7일 폐기된 검토 기록이다.
그 작업의 큐레이션 제외 문구는 이번 독립 기능의 금지 조건이 아니며, 기존 MVP 완료
판정이나 다른 후속 기능의 완료 상태를 소급 변경하지 않는다.
...
- 노래 클립 상세 요구사항·설계안·수용 기준: [노래 클립 요구사항과 구현 계획](../otw-play-singing-clips-requirements-and-plan.md). 이 문서의 DEC-081·FR-060~064를 구체화하는 하위 문서다.
...
| DEC-005 | 초기 MVP의 저장형 플레이리스트 제외 기록. | DEC-080으로 확장 | 2026-09-10 개인 비공개 저장을 포함한다. 재생 대기열과 영구 개인 목록은 별개다. |
...
| DEC-052 | PR-8 이후 최우선 프로그램은 playlist 벌크 catalog 수집과 회원 proposal 수정·철회다. | 완료된 우선순위 기록, 후속 순서는 DEC-079로 대체 | P0-A/B 기반 구현과 P0-C 운영 검증 기록은 유지한다. 다음 개발은 멤버 페이지 SEO·큐레이션이며 멤버 제작 참여를 SEO 선행 조건으로 두지 않는다. |
...
| DEC-058 | 추가 credit 범위는 OTW 멤버의 가창과 작품·편곡·제작 참여로 제한하고 외부 음악 관계자용 상세 credit·contributor graph는 만들지 않는다. | 범위 유지, 전달 순서는 DEC-079로 보완 | 멤버 페이지는 published 관계에서 파생하며 1곡부터 직접 URL, 3곡부터 navigation·SEO를 허용한다. 제작 참여·관리자 최대 5곡 pin은 낮은 우선순위로 둔다. 초기 페이지는 기존 가창·최근 공개 목록으로 성립하며 pin이나 contribution 모델을 선행 요구하지 않는다. |
...
| DEC-077 | 차기 주요 메뉴는 `발견 / 곡 탐색 / 멤버`로 나누되 현재 카탈로그·필터·상세·player를 재사용한다. | 폐기: 2026-09-07 | 기존 3개 화면 시안은 검토 기록으로 보존한다. 현재 구현 기준은 복원된 발견·곡 검색·우측 플레이어이며, 신규 큐레이션은 DEC-080을 따른다. |
...
| DEC-079 | 멤버 페이지 SEO·큐레이션을 높은 우선순위로, 멤버 제작 참여·대표곡 지정·참여 정보 정정을 낮은 우선순위로 둔다. | 확정: 다음 개발 순서·미구현 | SEO에 필요한 실제 멤버 페이지·공개 집계·직접 경로를 함께 구현한다. 방송 가창·개인 감상은 사용자용 OTW Play 내부 탭에서 관리하도록 후속 구체화한다. 기존 미충족 ADM-009·FR-021과 운영 검증은 독립 backlog로 보존하며 다음 두 기능의 선행 완료 조건으로 승격하지 않는다. |
| DEC-080 | 상단 플레이리스트 탭과 발견 요약에서 오리지널·커버·멤버별 전체 가창 모음을 제공하며 기본 클릭은 목록 보기로 이동하며 대기열 추가는 상세 화면의 명시적 버튼으로 실행한다. 회원은 독립 사본과 빈 목록을 편집하고 계정별 비공개로 저장한다. | 확정: 2026-09-10 구현 | 최근 대표 가창 24개 권장안을 폐기한다. vocal/featured_vocal 참여, 전체 페이지 수집, 명시적 저장과 서버 재조회, 소유권·version 충돌 정책은 구현 가이드 28절을 따른다. |


### 3.1 노래 클립 확장 기준안 — 2026-09-14

| ID | 결정 | 상태 | 영향 |
| --- | --- | --- | --- |
| DEC-081 | 공식 오리지널·커버와 방송 노래 클립은 관리자 작업 영역과 사용자 기본 탐색을 분리하되, 곡·가창·가창자·영상 소스의 공통 카탈로그를 공유한다. | 정책 확정; 코드·자동 검증 완료, 실사용 검증 잔여 | 2026-09-15 정책 변경: 방송 가창은 `relation_type=singing_clip`, `release_type=broadcast`로 고정한다. 공식 버전이 없는 곡도 클립 등록 시 공통 곡으로 생성할 수 있다. DEC-079·FR-059의 방송 가창 탭을 구체화하고, DEC-067의 비공개 draft 기반을 재사용한다. 공개 조건·초기 수록 범위는 상세안 8절의 사용자 확정 정책을 적용한다. |

확정된 DEC-081은 검수된 방송 가창의 명시적 게시를 허용하여 DEC-067의 draft 전용 제한을 대체한다. DEC-080의 공식 기본 모음에는 클립을 포함하지 않는다. 아래 초기 MVP 설명은 기존 공식 곡 구현 범위이며,
이번 후속 확장의 완료 여부와 분리한다.
...
곡과 가창 기록은 분리해 관리한다. 2026-09-15 확정 정책에 따라 방송 가창의
곡 관계는 노래 클립으로 고정하며 커버·오리지널 선택을 제공하지 않는다.
공식 버전과 방송 가창은 동일한 곡 정보를 공유할 수 있다.

| 분류 축 | 값 예시 | 적용 대상 |
| --- | --- | --- |
| 곡 관계 | 오리지널, 커버, 노래 클립(방송 가창에 고정) | 해당 가창 버전과 곡의 관계 |
...
DEC-079에 따라 다음 개발은 FR-043·044의 기존 가창 기반 페이지와 FR-046의 SEO다.
FR-044의 `만든 곡`, FR-045·047·048 및 ADM-031·032·037은 낮은 우선순위이며 삭제하지 않는다.
9월 7일 폐기된 전면 개편을 재개하지 않고 SEO에 필요한 실제 멤버 페이지를 최소 범위로 추가한다.
...
### 9.8 현재 기능을 재사용하는 3개 화면 [폐기된 차기 UI 검토 기록]
...
### 9.9 기본 큐레이션과 사용자 편집 [2026-09-10 구현]

사용자 확정 요구사항:

- FR-056: 상단 `발견 / 곡 검색 / 플레이리스트`와 발견의 `멤버로 찾기 → 플레이리스트 → 최근 공개된 곡`에서 기본 모음을 탐색한다. 오리지널·커버 두 큰 카드 다음에 현재 멤버별 가창곡, 로그인 회원의 내 플레이리스트를 표시한다.
- FR-057: 카드의 주 동작은 목록 보기로 이동한다. 상세 화면의 추가 버튼은 조건에 맞는 전체 가창을 기존 대기열 끝에 추가한다. 최신 큐에 performanceId로 중복 제거하며 기존 순서·반복·섞기를 보존한다. 전체 추가 완료 후 해당 목록의 첫 재생 가능한 곡을 처음부터 재생하며, 이미 큐에 있는 곡도 선택한다. 모든 페이지를 동일 revision으로 수집한 뒤 한 번 반영한다. 실패·충돌·취소·화면 이탈은 부분 반영하지 않는다.
- FR-058: 이번 구현에 회원 개인 편집·저장을 포함한다. 좌측 카탈로그 가창 검색, 우측 제목·설명·순서·삭제·드래그/키보드 정렬과 모바일 전환을 제공한다. 기본 목록 전체 사본은 가져온 시점에 고정된다. 계정별 비공개로 저장하고 소유자만 조회·수정·삭제한다. 저장 버튼 → 서버 재조회 이후 성공을 표시하며 충돌·실패는 입력을 보존한다.

- 대표곡·대표이미지 (2026-09-11): 개인 목록은 포함된 공개·재생 가능한 가창 중 썸네일이 있는 하나를 대표곡으로 지정한다. 순서 변경과 대표곡 선택은 독립이며 삭제 시 지정을 해제한다. 미지정·이용 불가 시 목록 순서상 첫 이용 가능한 썸네일을 사용한다. 철회된 기존 지정은 보존하고 교체 필요 상태로 안내한다.
- 관리자 기본 플레이리스트 (2026-09-11): `/admin/otw-play?tab=playlists`에서 기존 자동 목록의 이름·설명·대표곡만 편집한다. 선정 조건·순서는 유지한다. 미지정 시 오리지널/커버의 최신 가창 이미지 또는 멤버 이미지를 사용한다. 대표곡 ID와 별도 설정 버전을 저장하며 카탈로그 revision을 변경하지 않는다. 이번 구현은 로컬 검증까지이며 원격 migration·배포는 제외한다.

가창 relation으로 오리지널/커버를 판정하며 멤버 목록은 명시적 vocal 또는 featured_vocal만 포함한다. 동일 곡의 다른 가창은 각각 포함하고 동일 가창의 여러 소스는 한 항목이다. 0곡 멤버도 표시하고 곡 수와 가창 수를 구분한다. 공개·소스 선택 권위와 공개 flag를 유지하며 회원 인증만으로 비활성 공개를 우회하지 않는다.

철회·삭제된 가창 참조는 개인 목록에 이용 불가 항목으로 남긴다. 비공개 카탈로그 정보는 반환하지 않는다. 개인 응답 no-store, 개인 페이지 noindex, 계정 전환 시 캐시·편집 상태 제거를 적용한다. 구현과 검증 경계는 [구현 가이드 28절](otw-play-implementation-guide-before-2026-09-17.md#28-큐레이션-요구사항과-구현-계획)을 따른다.

### 9.10 방송 가창·개인 감상 [사용자용 OTW Play 내부 탭]

- FR-059: 방송 가창과 개인 감상 기능은 **사용자용 OTW Play 내부 탭**에서 탐색·관리한다.
  관리자 OTW Play 탭을 뜻하지 않는다는 사용자 답변을 반영했다. 상세 구조는 구현 가이드 29.4절을 따른다.
- 방송 가창은 기존 후보 수집·비공개 draft 검수 이후의 방송일·원본 방송·setlist·공개
  탐색/재생을 구체화한다. 사용자 탭은 공개된 가창을 소비하고 검수·게시 권한은 기존 관리자에 둔다.
- 개인 감상은 좋아요/즐겨찾기·최근 들은 곡·개인 저장 목록을 중심으로 먼저 설계하고,
  멤버 라디오·랜덤 재생은 후보 선정과 반복 정책을 별도로 정의한다. 행동 기반 개인화는 별도 검토다.
- 저장형 사용자 큐레이션은 개인 감상 목록과 같은 소유·저장 모델로 연결하고 중복 보관함을
  만들지 않는다. 개인 목록 schema/API·명시적 저장·충돌 정책은 28절에 확정했다. 감상 기록의 보관 기간과 공개/공유는 별도다.

### 9.11 노래 클립 카탈로그 [사용자 정책 확정]

- FR-060: 공식 곡과 노래 클립은 공통 곡 identity를 사용한다. 클립을 등록하려고 공식
  가창을 먼저 만들게 하지 않는다. 같은 곡의 다른 방송 가창은 별도 performance이며,
  같은 실제 가창의 편집 영상은 검수 후 해당 performance의 다른 source로 연결한다.
- FR-061: 사용자용 OTW Play의 노래 클립 탭은 공개된 방송 가창을 멤버·곡·방송일로
  탐색하고 가창별로 재생·큐 추가할 수 있게 한다. 공식 곡 기본 검색·집계·모음의 범위는
  유지하고, 공통 곡 상세에서는 공식 버전과 방송 가창을 구분해 연결한다.
- FR-062: 관리자는 공식 곡과 분리된 노래 클립 목록에서 URL 직접 등록 또는 기존 후보
  검수로 진입한다. 곡 검색·신규 곡 생성, 가창자·구간·출처 확인, draft 저장·재조회,
  명시적 게시·재조회까지 하나의 자연스러운 흐름으로 완료할 수 있어야 한다.
- FR-063: 채널 승인, 항목 검수·게시, 소스 재생 가능 상태는 독립적으로 검증한다.
  후보 자동 수집은 자동 게시가 아니며, 방송일과 클립 업로드일을 혼동하거나 제목만으로
  가창자·곡·방송일을 확정하지 않는다. 원본·날짜 미확인 항목의 공개 조건은 CLIP-TBD를 따른다.
- FR-064: 기존 단일 player와 세션 큐를 공유하고 선택한 가창의 구간을 재생한다.
  탭 전환으로 재생을 끊거나 같은 song의 다른 performance를 중복으로 제거하지 않는다.
  후보→draft→게시→공개 탐색→재생과 철회 이후 재조회를 실제 화면과 권위 데이터로 검증한다.

상세 요구사항, 제안된 1차 범위, 개인 저장 목록 연동 경계와 검증 시나리오는
[노래 클립 요구사항과 구현 계획](../otw-play-singing-clips-requirements-and-plan.md)에 둔다.
...
2026-09-14 기준 아래 목록은 전체 확장 방향이다. 첫 전달은 DEC-081의 노래 클립 단위
등록·검수·공개·재생을 기준안으로 삼고, 방송별 setlist·원본만으로 재생하는 경로는
별도 정책·범위 결정으로 분리한다. [상세 범위와 단계](../otw-play-singing-clips-requirements-and-plan.md)를 따른다.
...
2026-09-08 DEC-079가 다음 개발 순서의 권위다. 기존 P0~P4는 과거 프로그램 분류이며
멤버 제작 참여 → 정정 → SEO 또는 큐레이션 순서로 구현하도록 강제하지 않는다.
높은 두 기능 사이의 선후는 사용자가 확정하지 않았으므로 독립적으로 착수 가능한 상태로 준비한다.
...
| 높음 · NEXT-SEO | 개인 프로필·Play 멤버 SEO 구현 | 공통 metadata·곡/가창 수·목록·진입점·sitemap 구현. 공개 환경의 실제 재생 검증은 후속이며 가이드 30절에 증거·제한 기록 |
| 높음 · NEXT-CUR | 기본 큐레이션 | 기본 전체 가창 모음·대기열 일괄 추가·회원 편집/저장 구현. 현재 상태는 구현 가이드 28절 |
| 상세 기준안 문서화 · NEXT-BROADCAST | 노래 클립 관리·사용자 탭 | DEC-081·FR-060~064와 [노래 클립 상세안](../otw-play-singing-clips-requirements-and-plan.md)을 따른다. 공통 카탈로그, 분리된 관리·탐색, 클립 단위 공개·재생을 먼저 완성한다. 7개 수록·공개 정책 확정. 구현·검증 결과는 상세안 11절 참고 |
| 후속 구체화 · NEXT-LIBRARY | 개인 감상 사용자 탭 | 좋아요/즐겨찾기·최근 들은 곡·개인 목록·멤버 라디오를 구체화. 저장형 큐레이션과 통합하고 소유/보관/동기화 정책 결정 |
| 낮음 · LATER-CREDIT | 멤버 제작 참여 | FR-045·ADM-031, `만든 곡` 집계·공식 근거·관리자 검증과 표시 |
| 낮음 · LATER-PIN | 대표곡 지정 | FR-047·ADM-032, 관리자 최대 5곡 pin·순서 지정 |
| 낮음 · LATER-CORRECTION | 참여 정보 정정 | FR-048·ADM-037, 근거 기반 회원 정정·관리자 승인/거절 |
| 독립 backlog | 기존 요구사항 미충족 | ADM-009 가창 병합, FR-021 재생 패널 원곡 가수 표시. 사용자 지정 높은 기능의 선행 조건이 아니며 별도 보완 |
| 별도 검토 | 개인화·외부 생태계 | 행동 기반 추천·외부 서비스 연동은 별도 범위 결정. 자동 공개·AI 단독 곡 확정 금지는 유지 |
...
readback으로 완료한다. 이 운영 완료 기록은 당시 snapshot이며 이번 문서 정리로 최신
운영 상태를 재검증하지 않았다. P0-C는 개발 순서와 무관한 지속 운영 항목이다.
우선순위 변경은 DEC-079와 이 표, 구현 가이드 29절을 함께 갱신한다.
...
### 14.2 큐레이션 수용 기준 — 구현 전

1. 기존 발견 화면에서 멤버 → 큐레이션 → 최근 곡 순서로 자연스럽게 접근한다.
2. 카드 클릭 → 실제 카탈로그 조회 → 기존 대기열 끝에 추가 → 추가 결과 안내를 완료한다.
3. 재생 중인 곡·위치·기존 순서를 보존하고, 빈 대기열에서는 명시적 재생 전 소리가 나지 않는다.
4. 내부/기존 중복, 다른 가창 버전, 전체 중복, 정상 0곡, 일부 재생 불가, 통신 실패,
   연속 클릭 및 조회 중 대기열 편집을 구별해 처리한다.
5. 추가된 항목의 제목·버전·순서를 실제 대기열에서 확인하고 삭제·재정렬·세션 복원한다.
6. 공개 비활성·관리자 preview·철회된 가창·소스 교체에서도 기존 접근 및 재생 정책을 유지한다.
7. 사용자 영구 저장 단계의 완료는 본인 계정 저장 → 서버 권위 재조회 → 재방문 복원과
   타인 수정 차단까지 확인해야 한다. 초기 세션 큐만으로 사용자 저장 완료를 주장하지 않는다.

이 기준은 구현 계획이며 검증 완료 기록이 아니다. 상세 검증 항목은 구현 가이드 28.7절에 둔다.

### 14.3 노래 클립 확장 수용 기준

DEC-081·FR-060~064는 [상세안의 CLIP-AC-01~12](../otw-play-singing-clips-requirements-and-plan.md#9-수용-기준과-검증-계획)로 검증한다.
기존 곡 재사용·클립만 있는 새 곡·기존 draft 보완·반복 가창·대체 소스·게시/철회·구간 재생·
공식 범위 격리·CAS·모바일·미확인 정보 정책을 포함한다. 개인 저장 목록 검증은 초기 범위에
포함할 때 적용한다. 실제 관리자 등록·게시와 사용자 탭 탐색·재생 및 권위 재조회가 완료되기
전에는 테스트나 draft 생성만으로 기능 완료를 기록하지 않는다.
...
| TBD-020 | 해결: 기본 구성 | 조건형 오리지널·커버 + 현재 멤버 순서의 명시적 vocal/featured_vocal 전체 가창. 0곡 멤버 포함 |
| TBD-021 | 해결: 조회 범위 | 최근 가창 공개순/ID 고정, 전체 가창 대상. 페이지·resolve 묶음 최대 60, revision 일치·취소·일괄 큐 반영 |
| TBD-022 | 해결: 편집·저장 | 기본 목록의 고정 사본 또는 빈 목록, 계정별 비공개 D1 저장, 명시적 저장/재조회, expectedVersion 충돌과 미저장 보호 |
| TBD-023 | 이번 범위 제외 | 운영자 선곡·수정, 태그/분위기 큐레이션, 공유·공동 편집은 후속 별도 결정 |
| TBD-024 | 방송 가창 사용자 탭 상세 | DEC-081·FR-060~064로 공통 데이터·분리 탐색과 클립 단위 흐름을 문서화했다. 확정 경로·수록/공개 정책은 [정책 목록](../otw-play-singing-clips-requirements-and-plan.md#8-확정-정책)을 따른다. setlist는 후속 범위이며 운영 공개는 별도 절차다 |
| TBD-025 | 개인 감상 사용자 탭 상세 | 탭 위치는 사용자용 OTW Play 내부로 확정. 좋아요/즐겨찾기 통합 여부, 최근 재생 인정 기준·보관/삭제, 로그인·동기화·공유를 후속 결정 |
...
| 날짜 | 변경 |
| --- | --- |
| 2026-09-14 | DEC-081·FR-060~064와 노래 클립 상세 요구사항·구현 계획을 추가했다. 공통 카탈로그 재사용, 관리·탐색 분리, 실제 등록부터 공개 재생까지의 수용 기준을 문서화했다. 미결정 공개 정책은 유지하며 코드·DB·배포 변경은 없다. |
| 2026-09-10 | DEC-080·FR-056~058·TBD-020~023을 승인된 전체 가창·개인 비공개 저장 계약으로 갱신했다. 로컬 구현/검증, 원격 migration·운영 flag·배포 제외. |
| 2026-09-08 | DEC-079로 멤버 페이지 SEO·큐레이션을 다음 개발로 상향하고 제작 참여·대표곡 지정·참여 정보 정정을 낮은 우선순위로 분리했다. FR-059와 TBD-024/025에 방송 가창·개인 감상의 사용자용 OTW Play 내부 탭 방향을 기록했다. SEO를 낮은 기능에서 분리하고 문서 closeout·착수 인계 범위를 확정했다. 기능 구현·배포는 수행하지 않았다. |
| 2026-09-08 | DEC-080, FR-056~058, 큐레이션 수용 기준과 TBD-020~023을 추가했다. 위치·대기열 끝 추가·후속 편집을 확정 요구사항으로 기록하고 조건형 우선 등은 권장안으로 분리했다. DEC-077은 9월 7일 폐기 기록과 일치시켰다. 구현·운영 변경은 없다. |
