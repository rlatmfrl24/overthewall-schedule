...
과거 이미지: 390px 공유 크기 비교 — 당시 경로 `.tmp/snapshot-poster-qa/390px-editions.png` (현재 파일 없음).
...
과거 이미지: 수정 전후 390px 비교 — 당시 경로 `.tmp/snapshot-revision/comparison-390.png` (현재 파일 없음).
...
과거 이미지: 참고 이미지와 기존 편성표 비교 — 당시 경로 `.tmp/snapshot-legacy/comparison-544.png` (현재 파일 없음).
