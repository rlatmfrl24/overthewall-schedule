...
- `방송`, `미정` 일정 중 시작 시각이나 제목이 비어 있는 일정
- `게릴라` 일정은 자동 수집의 매칭·수정·중복 충돌 대상에서 제외하고 그대로 보존
...
8. 관리자가 승인하면 세션 시작 시각을 가장 가까운 30분 단위로 반올림하고,
   승인 시점에도 비어 있는 필드만 채우며 기존 값과 상태를 덮어쓰지 않음.
   새 일정 생성에만 `status="방송"`을 사용하고 같은 날짜의 `게릴라` 일정은 유지
...
- **게릴라 보존**: 게릴라는 자동 매칭·빈 대상·동등 일정·시간 충돌에서
  제외하여 자동 승인으로 수정되거나 신규 방송 생성을 막지 않음
