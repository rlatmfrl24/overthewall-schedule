...
5. VOD 처리 로직:
   - 해당 날짜에 시작 시간이 비슷한(±30분) 스케줄이 없으면 → 새 스케줄 생성
   - 시작 시간이 비슷한 스케줄이 있고, 상태가 "방송"이 아니거나 제목이 없으면 → 업데이트
   - 시작 시간이 비슷한 스케줄이 있고, 이미 "방송" 상태이며 제목이 있으면 → 변경 없음
6. **중요**: 한 날짜에 여러 개의 다시보기가 있으면 모두 별도의 스케줄로 생성됨
