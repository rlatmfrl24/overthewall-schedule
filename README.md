...
`pnpm d1:seed:local`은 fixture 전용 명령입니다. 실제 멤버 등 기존 로컬
데이터가 있으면 중단되며, fixture 환경이 필요할 때는
`pnpm d1:reset:local`로 빈 스키마를 만든 뒤 실행합니다. 기존 데이터를
의도적으로 fixture로 교체하는 경우에만 `pnpm d1:seed:local -- --force`를
사용합니다.
