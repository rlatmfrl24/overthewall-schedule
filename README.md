...
- **주요 공지사항 및 컨텐츠 정보 표시**: 공식 카페나 트위터의 주요 공지를 대시보드에 노출.
- **멤버별 컨텐츠 정보 표시**: 각 멤버의 정기 컨텐츠나 유튜브 업로드 소식 제공.
- **멤버 상세 페이지 (Member Detail Page)**: 프로필, 팬덤명, 데뷔일, 생일 등 상세 정보 및 SNS 링크 모음.
- **유닛별 필터링 (Unit Filtering)**: 특정 유닛(Unit) 멤버들의 일정만 모아보는 필터 기능.
- **기념일 알림 (Anniversary Alerts)**: 멤버 생일 및 데뷔 기념일 D-Day 표시 및 알림.
- **다크 모드 토글 (Dark Mode Toggle)**: 사용자 선호에 따른 테마 전환 기능.
- **관리자 대시보드 (Admin Dashboard)**: 일정 등록/수정 권한 관리 및 로그 시스템.
