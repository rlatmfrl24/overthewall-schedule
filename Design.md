...
- `/multiview`는 화면 높이가 우선인 특수 화면으로, 사이트 푸터를 숨기고 남은
  높이를 iframe에 배정한다.
- 상단에는 CHZZK 채널이 등록된 멤버 칩만 한 줄로 배치하고, 선택한 채널 조합을
  `Mul.Live` URL로 만들어 하단 iframe에 표시한다.
- 멤버 색상은 칩의 선택 상태와 테두리 accent에만 사용하고, 본문 배경은 semantic
  token을 따른다.
...
- 스냅샷은 공유 이미지 생성용이므로 고정 폭, 안정적 렌더링, 큰 텍스트 대비를
  우선한다.
- 스냅샷 헤더는 로고, 브랜드/날짜 메타, 제목을 분리해 날짜가 일정 데이터와
  혼동되지 않게 한다.
